...
class phononPost:
    ...
    def plot_band(self):
        ...
        ax.set_ylabel("Frequency (THz)", fontdict=font)
        ...
        plt.savefig(osp.join(self.filepath, "phonon.png"), dpi=400, bbox_inches = 'tight', pad_inches=0.1)
        plt.close()
    ...
    def phonon_dos_data_processing(self):
        ...
        for i in range(len(self.element)):
            ...
            for j in range(self.num_element[i]):
                ...
                count = count + 1
            phondos.append(tmp)
        # return data[:, 0]/33.35641, phondos, data[:, 1] 
    ...
    def phonon_dos_plotting(self):
        ...
        ax.set_xlabel("Frequency (eV)", fontdict=font)
    ...
    def phonon_band_dos_plotting(self):
        ...
        # ax.set_ylim(self.yminvalue, np.max(x)*1.05)
        # ax.set_ylim(self.yminvalue, np.max(x))
        egg = np.max(x)+2
        ...
        ax1.set_ylabel("Frequency [meV]", fontdict=font)
        ax1.set_ylim(self.yminvalue, egg)
        ...
        plt.savefig(osp.join(os.getcwd(), "phon_banddos.pdf"), dpi=300, bbox_inches='tight', pad_inches=0.05)
        plt.close()
